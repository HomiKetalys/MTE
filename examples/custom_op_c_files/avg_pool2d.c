    ...
    int8_t *output,const int32_t output_h, const int32_t output_w)
{
    ...
    const int32_t win_ele_nums=kernel_w*kernel_h;
    for(ch = 0; ch < input_ch; ch++){
        for(h = 0; h < output_h; h++){
            for(w = 0; w < output_w; w++){
                int32_t in_w=w*stride_h,in_h=h*stride_w;
                int32_t pad_avg=(in_h<pad_h_low||input_h+pad_h_low<in_h+kernel_h||in_w<pad_w_low||input_w+pad_w_low<in_w+kernel_w)?input_offset:act_min;
                int32_t win_sum =0;
                int32_t pad_count=win_ele_nums;
                ...
                for(win_h=h_begin;win_h<h_end;win_h++){
                    ...
                    for(win_w=w_begin;win_w<w_end;win_w++){
                        win_sum +=input[(win_h*input_w+win_w)*input_ch+ch];
                        pad_count--;
                    }
                }
                win_sum +=pad_avg*pad_count;
                int32_t avg=win_sum/win_ele_nums;
                avg = MAX(avg, act_min);
                avg = MIN(avg, act_max);
                output[(h * output_w+w) * input_ch + ch] = (int8_t)avg;
            }
        }
    }
}
...
void DW_CONV_FUNCTION_KERNEL(3,3,2,2,1,1,ssat)(
...
void DW_CONV_FUNCTION_KERNEL(3,3,2,2,1,1,minmax)(
    const int8_t *input, int32_t input_h, int32_t input_w,int32_t padded_input_w,
    const int8_t *weight, int32_t scale, int32_t bias,
    int32_t act_min, int32_t act_max,
    int8_t *output, int32_t output_h, int32_t output_w, int32_t output_ch, int32_t output_offset);

void DW_CONV_FUNCTION(3,3,2,2,1,1)(
    //input
    const int8_t *input,
    ...
    const int32_t output_offset)
{
    ...
    if (act_min == -128 && act_max == 127)
        dw_conv_kernel_func = DW_CONV_FUNCTION_KERNEL(3,3,2,2,1,1,ssat);
    else
        dw_conv_kernel_func = DW_CONV_FUNCTION_KERNEL(3,3,2,2,1,1,minmax);
    ...
    int8_t *output, const int32_t output_h, const int32_t output_w, const int32_t output_ch, const int32_t output_offset)
{
    ...
    for (i = 0; i < output_h; i++) {
        ...
        for (j = 0; j < output_w; j += 4) {
            ...
            for (k = 0; k < 3; k++) {
                for (v = 0; v < 3; v++) {
                    ...
                    sum1 += input_window[v + 2] * weight[3 * k + v];
                    sum2 += input_window[v + 2 * 2] * weight[3 * k + v];
                    sum3 += input_window[v + 3 * 2] * weight[3 * k + v];
                }
        ...
        input += 2 * padded_input_w-input_w;
    }
}

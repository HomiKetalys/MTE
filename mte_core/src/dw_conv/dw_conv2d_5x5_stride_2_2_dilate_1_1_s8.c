...
void DW_CONV_FUNCTION_KERNEL(5,5,2,2,1,1,ssat)(
...
void DW_CONV_FUNCTION_KERNEL(5,5,2,2,1,1,minmax)(
    const int8_t *input, int32_t input_h, int32_t input_w,int32_t padded_input_w,
    const int8_t *weight, int32_t scale, int32_t bias,
    int32_t act_min, int32_t act_max,
    int8_t *output, int32_t output_h, int32_t output_w, int32_t output_ch, int32_t output_offset);

void DW_CONV_FUNCTION(5,5,2,2,1,1)(
    //input
    const int8_t *input,
    ...
    const int32_t output_offset)
{
    ...
    if (act_min == -128 && act_max == 127)
        dw_conv_kernel_func = DW_CONV_FUNCTION_KERNEL(5,5,2,2,1,1,ssat);
    else
        dw_conv_kernel_func = DW_CONV_FUNCTION_KERNEL(5,5,2,2,1,1,minmax);
        ...
        for (j = 0; j < output_w; j += 4) {
            ...
            for (k = 0; k < 5; k++) {
                for (v = 0; v < 5; v++) {
                    ...
                    sum1 += input_window[v + 2] * weight[5 * k + v];
                    sum2 += input_window[v + 2 * 2] * weight[5 * k + v];
                    sum3 += input_window[v + 3 * 2] * weight[5 * k + v];
                }
